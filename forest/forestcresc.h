...
#ifndef FOREST_FORESTCRESC_H
#define FOREST_FORESTCRESC_H
...
public:
   ...
   */
  Nodecresc(unsigned int treeChunk) :
...
template<typename NodeType>
class ForestCresc {
  unique_ptr<Nodecresc<NodeType> > nodeCresc; // Crescent node block.
  unique_ptr<FBCresc> fbCresc; // Crescent factor-summary block.

  vector<IndexT> leafNodes; // Tree-relative indices of leaves.
...
public:
  ...
  ForestCresc(unsigned int treeChunk) :
    nodeCresc(make_unique<Nodecresc<NodeType>>(treeChunk)),
  ...
  void treeInit(unsigned int tIdx,
                IndexT nodeCount) {
    nodeCresc->treeInit(tIdx, nodeCount);
  }
