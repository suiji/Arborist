...
In a 2015 paper (https://www.jstatsoft.org/article/view/v077i01/v77i01.pdf), Wright and Ziegler compare several *R*-language implementations of the Random Forest algorithm, including Ranger and Rborist.  A key finding was that Rborist failed to achieve the *O*(sqrt(p)) scaling expected for classification under high predictor (feature) count, *p*.  Although Rborist was performing the expected *O*(sqrt(p)) splitting operations, performance was being eclipsed by greedy repartitioning, which was being performed on every node/predictor cell.  As a result of the authors' findings, repartitioning was revised the following year to a lazy scheme, operating only on those cells known to be about to split.  Using an 8-bit repartitioning history, the lazy scheme should scale appropriately beyond *p* = 10^4.  A 16-bit history can be substituted to bring scaling well beyond *p* = 10^6. Although not explicitly mentioned, their work illustrates that data movement poses a larger obstacle to scaling than does splitting.  Benchmarks used in the study are provided alongside the paper at https://www.jstatsoft.org/article/view/v077i01.
