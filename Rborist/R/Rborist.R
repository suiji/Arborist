...
Rborist <- function(x, y, ...) UseMethod("Rborist")
...
Rborist.default <- function(x,
  ...
  # Quantile constraints:  regression only
    ...
    sampler <- tryCatch(.Call("rootSample", preFormat, argList), error = function(e){stop(e)})
    rfDeep(preFormat, sampler, argList)
}


rfDeep <- function(preFormat, sampler, argList) {
    train <- tryCatch(.Call("rfTrain", preFormat, sampler, argList), error = function(e){stop(e)})
    ...
    if (argList$noValidate) {
        ...
        summaryValidate <- validateDeep(train, preFormat, sampler, argList$impPermute, argList$ctgCensus, argList$quantVec, argList$quantiles, argList$trapUnobserved, argList$nThread, argList$verbose)
    }
